...
def extract_room_outlines_from_grid(grid, rooms_data):
    ...
    for room in rooms_data:
        ...
        # Find all connected floor cells for this room
        cells = find_room_cells(grid, center_x, center_y)
        outline = trace_room_outline(cells)
...
def find_room_cells(grid, center_x, center_y):
    """Find all cells belonging to a room using flood fill"""
    height, width = grid.shape
    start_x, start_y = int(center_x), int(center_y)
    
    # Find a valid starting point near center
    for dy in range(-3, 4):
        for dx in range(-3, 4):
            check_x, check_y = start_x + dx, start_y + dy
            if (0 <= check_x < width and 0 <= check_y < height and 
                grid[check_y, check_x] in [1, 3, 5, 6, 7]):  # Floor-type cells
                start_x, start_y = check_x, check_y
                break
    
    # Flood fill to find all connected room cells
    visited = set()
    ...
    while stack:
        ...
        if (0 <= x < width and 0 <= y < height and 
            grid[y, x] in [1, 3, 5, 6, 7]):  # Floor-type cells
            cells.add((x, y))
...
def trace_room_outline(cells):
    """Create outline coordinates from room cells"""
    if not cells:
        return []
    ...
    return [[x, y] for x, y in sorted(perimeter)]
        ...
        for room in rooms_data:
            ...
            if room_id in room_outlines:
                ...
                room['shape_data'] = {
                    ...
                    'is_rectangular': len(set(outline_data['cells'])) == (
                        (max(x for x, y in outline_data['cells']) - min(x for x, y in outline_data['cells']) + 1) *
                        (max(y for x, y in outline_data['cells']) - min(y for x, y in outline_data['cells']) + 1)
                    ) if outline_data['cells'] else False
                }
